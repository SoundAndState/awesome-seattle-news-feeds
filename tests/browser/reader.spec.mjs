...
async function search(page,text) {await page.locator('#search').fill(text);}
// Click the visible sticky control without Playwright scrolling its ancestor first.
async function clickVisibleMode(page,mode) {const box=await page.locator(`[data-mode="${mode}"]`).boundingBox();await page.mouse.click(box.x+box.width/2,box.y+box.height/2);}
...
test('Back and Forward restore filters, searches, source directory and reloads',async({page})=>{
  ...
  await chooseSource(page,'transit-news');await page.locator('#search').pressSequentially('Seattle parks');await page.locator('#search').blur();
...
test('preview history restores scroll and keyboard focus; direct links close locally',async({page})=>{
  ...
  await openMenu(page);await page.getByRole('button',{name:'About this reader'}).click();await page.goBack();await expect(page.locator('#about-dialog')).toBeHidden();await expectResourceFocus(page,'#about-button');
...
test('backup restores article and post saves into a fresh library',async({page,newReaderPage})=>{
  ...
  await openMenu(page);await page.locator('#about-button').click();const downloadPromise=page.waitForEvent('download');await page.locator('#export-state').click();const backup=await (await downloadPromise).path();
  const fresh=await newReaderPage();await load(fresh,{fail:true});await openMenu(fresh);await fresh.locator('#about-button').click();await fresh.locator('#backup-file').setInputFiles(backup);await expect(fresh.locator('#backup-status')).toContainText('The reader combined 2 saved items');
...
test('unreadable backups explain recovery without changing the library',async({page})=>{
  await load(page);await page.locator('.save-button').first().click();await openMenu(page);await page.locator('#about-button').click();
...
test('mode switches restore independent filters, searches and reading positions',async({page})=>{
  ...
  await clickVisibleMode(page,'posts');await expect(page.locator('.post')).toHaveCount(socialCount);await search(page,'parks');await page.locator('#search').blur();
  await clickVisibleMode(page,'articles');await expect(page.locator('#filter-chips')).toContainText('Seattle & regional');await expect(page.locator('#search')).toHaveValue('Seattle');expect(Math.abs(await page.evaluate(()=>scrollY)-y)).toBeLessThan(5);
  await clickVisibleMode(page,'posts');await expect(page.locator('#search')).toHaveValue('parks');await expect(page.locator('.story h2')).toHaveCount(0);await expect(page.locator('.post-text').first()).toContainText('Parks & trails <3');
});
...
test('metadata follows article titles and archives remain private until clicked',async({page})=>{
  ...
  await expect(story.locator('.publisher')).toHaveRole('button');
...
test('cards and previews label full dates, including update-only and date-only entries',async({page,newReaderPage})=>{
  ...
  await page.goto(`./#view=all&source=${id}`);await expect(page.locator('.story')).toHaveCount(1);
  ...
  await page.locator('.story .save-button').click();const download=page.waitForEvent('download');await page.locator('#saved-button').click();await page.locator('#export-saved').click();const csv=await readFile(await(await download).path(),'utf8');expect(csv).toContain('"Read","Updated"');expect(csv).toContain('2026-09-15T12:45:00.000Z');
...
test('legacy Bluesky links resolve to Posts with a scoped account picker',async({page})=>{
  await load(page);await page.goto('./#section=bluesky');await expect(page.locator('.post')).toHaveCount(socialCount);await expect(page).toHaveURL(/#mode=posts&view=all$/);await page.locator('#filter-button').click();await expect(page.locator('#section-filter')).toBeHidden();await expect(page.locator('.source-choice')).toHaveCount(socialCount);
  const id=catalog.feeds.find(f=>f.category==='bluesky').id;await page.locator(`[data-source="${id}"]`).click();await expect(page.locator('.post')).toHaveCount(1);await page.goto(`./#view=all&source=${id}`);await expect(page.locator('[data-mode="posts"]')).toHaveAttribute('aria-pressed','true');await expect(page.locator('.post')).toHaveCount(1);
});
...
test('feed list guide stays open when the catalog finishes loading',async({page})=>{
  ...
  await page.goto('./',{waitUntil:'domcontentloaded'});await openMenu(page);await page.locator('#feed-list-button').click();await expect(page.locator('#feed-list-dialog')).toBeVisible();
  ...
  await page.goBack();await expect(page.locator('#feed-list-dialog')).toBeHidden();await expectResourceFocus(page,'#feed-list-button');
});
...
test('feed list guide fits desktop, phone, landscape and expanded text',async({page})=>{
  ...
  for(const size of [{width:1280,height:900},{width:320,height:568},{width:390,height:844},{width:760,height:360}]) {
    await page.setViewportSize(size);await openMenu(page);await page.locator('#feed-list-button').click();const dialog=page.locator('#feed-list-dialog');
    ...
    await dialog.locator('.feed-list-content').evaluate(node=>node.scrollTop=node.scrollHeight);await expect(dialog.locator('.close-button')).toBeInViewport();
  ...
  await openMenu(page);await page.locator('#feed-list-button').click();const dialog=page.locator('#feed-list-dialog');
  expect(await dialog.evaluate(node=>node.scrollWidth<=node.clientWidth)).toBe(true);
...
test('phone and landscape dialogs retain close controls and expanded text reflows',async({page})=>{
  await load(page);for(const size of [{width:320,height:568},{width:390,height:844},{width:760,height:360}]){
    await page.setViewportSize(size);await openMenu(page);await page.locator('#about-button').click();const dialog=page.locator('#about-dialog');expect((await dialog.boundingBox()).height).toBeGreaterThan(size.height-65);
    await dialog.locator('.about-content').evaluate(n=>n.scrollTop=n.scrollHeight);await expect(page.getByRole('button',{name:'Close about'})).toBeInViewport();await page.keyboard.press('Escape');await expectResourceFocus(page,'#about-button');
  }
  ...
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await page.keyboard.press('Escape');await page.locator('#filter-button').click();await expect(page.getByRole('button',{name:'Close filters'})).toBeInViewport();expect(await page.locator('#filter-dialog').evaluate(n=>n.scrollWidth<=n.clientWidth)).toBe(true);
});
